...
S_priobr = priob_nach_1[1]*I_g*(1-((type_ir['текущий'].iloc[0]-1)
                                   /
                                   (type_ir['планируемый'].iloc[0])))
print("S приор к tk году = ",round(S_priobr.values[0], 3), "правая часть",(1-((type_ir['текущий'].iloc[0]-1)
                                   /
                                   (type_ir['планируемый'].iloc[0]))))
...
goda = info_1.iloc[2]
price = info_1.iloc[4:9].astype(float)
print(price)
...
for g in goda.iloc[1:len(goda)]:
    S_nak.append(S_baz[g]+S_nak[count-1]*k[(count-1)])
    count+=1
print(S_nak)

S_raz = S_nak[-1]*koef_pr(koef_ir, type_ir, 0) * (1-((type_ir['текущий'].iloc[0]-1)
                                   /
                                   (type_ir['планируемый'].iloc[0])))
print("S разр к tk году = ", round(S_raz.values[0], 3))


print("1 ИР = ", round(S_priobr.values[0] + S_raz.values[0] +  info_1['info1'].iloc[-1],3))
price_all_ir[0] = (round(S_priobr.values[0] + S_raz.values[0] +  info_1['info1'].iloc[-1],3))
...
for i in range(len(test)-1):
    if test[i] != 0:
        for j in range(i+1, len(test)):
            if test[j] != 0:
                a = round((j - i) * math.sqrt(test[j] / test[i]), 3)
                rang_first.append(i)
                rang_last.append(j)
                dEk.append(a)
